...
function [X,y] = nearcorr_newton(G,b,tau,tol)
...
%  fprintf('\n ---------------------------------------------------------');
%  fprintf('\n  iter     pobj          dobj            relgap          etaorg        eta      time | cg_iter');
...
while (eta > error_tol & k< Iter_Whole) %% based on the relative gradient for academic research
 ...
 %fprintf('Newton-CG: Number of CG Iterations == %d \n', iterk)
  ...
  if (flag~=0); % if CG is unsuccessful, use the negative gradient direction
     % d =b0-Fy;
     %disp('..... Wrong: Not a completed Newton-CG step ......')
  end
 slope = (Fy-b0)'*d; %%% nabla f d
     ...
     time_used = etime(clock,t0);
      Res_b(k) = norm_b;
      %fprintf('\n   %d   %- 5.4e     %-5.4e      %-3.2e        %3.2e     %3.2e     %3.1f | %d', ...
     %k, val_obj, val_dual, gap, norm_b, eta, time_used, iterk);
